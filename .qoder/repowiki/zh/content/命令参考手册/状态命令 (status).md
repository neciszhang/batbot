...
PROV_SPEC --> TPL_HB["心跳模板<br/>src/templates/HEARTBEAT.md"]
PROV_SPEC --> TPL_TOOLS["工具使用模板<br/>src/templates/TOOLS.md"]
...
- **提供商注册表**：定义支持的 LLM 提供商规格，包括名称、关键词、环境变量键等元数据
...
## 架构总览
下图展示了 status 命令从注册到执行的完整调用链路，以及与配置、日志、提供商注册表的关系。
...
## 结论
status 命令已从简单的占位符实现为功能完整的系统状态检查工具。通过提供配置文件验证、工作区状态检查、模型配置显示和提供商状态枚举等功能，用户可以全面了解 batbot 系统的运行状态。建议用户定期使用 status 命令进行系统健康检查，并根据输出信息及时发现和解决问题。随着系统的不断发展，status 命令还将继续扩展更多监控和诊断功能，为用户提供更好的系统管理体验。

## 附录
- **版本信息**：当前版本为 1.0.0，使用蝙蝠图标作为标识
- **CLI 使用**：通过 `batbot status` 命令触发状态检查
- **状态输出**：包含配置状态、工作区状态、模型配置和提供商状态四部分信息
- **扩展建议**：未来可以增加资源使用情况、服务状态等更多监控指标
