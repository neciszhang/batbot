...
该系统以命令行入口为核心，通过初始化流程创建并填充工作空间，同时提供模板文件用于指导代理行为、记录长期记忆、管理周期任务等。
...
- 工具函数：工作空间模板同步，确保工作空间目录下存在所需模板与子目录  
...
- 同步模板到工作空间：复制内置模板、创建记忆子目录与历史文件、创建技能目录  
...
图表来源
- [src/cli.ts:22-58](file://src/cli.ts#L22-L58)
...
章节来源
- [src/config/schema.ts:108-117](file://src/config/schema.ts#L108-L117)
- [src/utils/helpers.ts:11-47](file://src/utils/helpers.ts#L11-L47)
...
- 创建与初始化  
  - 执行 onboard，系统会：  
    - 生成默认配置（若不存在）  
    - 创建工作空间目录（若不存在）  
    - 同步模板到工作空间（含 memory 与 skills 目录）  
- 维护  
...
章节来源
- [src/config/schema.ts:130-146](file://src/config/schema.ts#L130-L146)
- [src/config/paths.ts:4-10](file://src/config/paths.ts#L4-L10)
- [src/cli.ts:22-58](file://src/cli.ts#L22-L58)