...
### 安装与环境准备
- Node.js 版本要求
  - 项目使用 TypeScript 5.x 与 Node.js CommonJS 模式，建议使用 Node.js LTS（如 18.x 或 20.x）以获得最佳兼容性。
- 安装步骤
...
- 工作空间设置
  - agents.default.workspace 指定工作空间根目录，默认为 ~/.batbot/workspace。
  - 可通过相对路径或绝对路径配置。
- 配置模式参考
...
章节来源
- [src/cli.ts:22-93](file://src/cli.ts#L22-L93)
...
## 结论
通过以上步骤，你可以在 15 分钟内完成 BatBot 的安装、初始化配置、工作空间搭建与首次对话准备。建议在完成 API 密钥配置后，结合模板文件进一步定制个性化体验。
